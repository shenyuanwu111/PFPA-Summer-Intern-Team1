...
from .kmv_em import build_kmv_summary, estimate_kmv_em
...
def run_pipeline(
    ...
    kmv_estimates, kmv_result = estimate_kmv_em(matched)
    kmv_summary = build_kmv_summary(kmv_estimates, match.symbol, kmv_result)
    data_dictionary = build_data_dictionary(kmv_estimates)
    ...
    paths = {
        ...
        "kmv_csv": prefix.with_name(f"{match.symbol}_kmv_em.csv"),
        "kmv_summary_csv": prefix.with_name(f"{match.symbol}_kmv_summary.csv"),
    ...
    kmv_estimates.to_csv(paths["kmv_csv"], index=False)
    kmv_summary.to_csv(paths["kmv_summary_csv"], index=False)
    save_workbook(
        paths["excel"],
        prices,
        financials,
...
def run_batch_pipeline(
    ...
    """Run the full data and KMV EM workflow for multiple companies."""

    all_paths: dict[str, dict[str, Path]] = {}
    summary_frames = []
    for company in companies:
        paths = run_pipeline(company, start, end, out_dir, rates=rates, user_agent=user_agent)
        symbol = Path(paths["kmv_summary_csv"]).name.split("_", 1)[0]
        all_paths[symbol] = paths
        summary_frames.append(pd.read_csv(paths["kmv_summary_csv"]))

    if summary_frames:
        summary = pd.concat(summary_frames, ignore_index=True)
        summary_path = Path(out_dir) / "assigned_companies_kmv_summary.csv"
        summary.to_csv(summary_path, index=False)
        all_paths["_batch"] = {"kmv_summary_csv": summary_path}
    return all_paths
...
def build_data_dictionary(matched: pd.DataFrame) -> pd.DataFrame:
    descriptions = {
        ...
        "kmv_asset_value": "Latent firm asset value inferred by inverting the Merton equity call equation.",
        "kmv_asset_return": "Daily log return of inferred KMV asset value.",
        "kmv_asset_volatility": "Annualized asset volatility estimated by the EM-style fixed-point loop.",
        "kmv_distance_to_default": "Merton/KMV distance to default over the selected horizon.",
        "kmv_pit_pd": "Point-in-time default probability proxy: standard-normal tail probability of distance to default.",
        "kmv_em_converged": "Whether the asset-volatility EM loop converged within tolerance.",
        "kmv_em_iterations": "Number of EM iterations used for the final KMV estimate.",
...
def save_workbook(
    ...
    kmv_estimates: pd.DataFrame,
    kmv_summary: pd.DataFrame,
    ...
    summary = pd.DataFrame(
        [
            ...
            ["Rows in KMV EM panel", len(kmv_estimates)],
            ["Financial statement match key", "SEC filing date, with fiscal period end as fallback"],
            ["Purpose", "KMV EM, point-in-time default probability, and TTC-style credit-risk inputs"],
        ],
        columns=["Field", "Value"],
    )
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        ...
        kmv_summary.to_excel(writer, sheet_name="KMV Summary", index=False)
        kmv_estimates.to_excel(writer, sheet_name="KMV EM", index=False)
